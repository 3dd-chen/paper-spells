...
async def get_access_token(settings: Settings, http_client: HttpClientInterface, token_store: dict = None) -> str:
    ...
    _token_locks = {l: lock for l, lock in _token_locks.items() if not l.is_closed()}
    
    loop = asyncio.get_running_loop()
    if loop not in _token_locks:
        _token_locks[loop] = asyncio.Lock()
    lock = _token_locks[loop]
    ...
    async with lock:
        # Check again inside the lock, as another concurrent task might have fetched the token
        now = int(time.time())
        if token_store and token_store.get("token") and now < token_store.get("expiry", 0) - 60:
            return token_store["token"]
